import './style.css';

export const Education = () => {
  return (
    <>
      <div className="education-container">
        <h1>Výcvik Českých dul</h1>
        <p>
          Láká vás stát se dulou? Provázet ženy těhotenstvím, být jim podporou
          během porodu, stát při nich v šestinedělí? Být rodinám oporou nejen ve
          chvílích štěstí a radosti, ale také ve složitých, napjatých,
          nečekaných či smutných okamžicích? Chcete se naučit, jak pečovat nejen
          o druhé, ale také o sebe? Jak pracovat s hranicemi, s vlastní
          kapacitou? Jak být dobrou, profesionální podporou druhým?
        </p>
        <p>
          Nabízíme vám dvouletý plnohodnotný výcvik, který vám pro práci duly
          poskytne pevné základy. Scházíme se každých cca pět týdnů na celý
          víkend. A to buď v našem krásném výcvikovém centru v srdci Prahy anebo
          online u vás doma (bližší info níže).
        </p>
        <button>Přihlásit</button>

        <img src="img/pexels.jpg" alt="" width="150px" />

        <p className="reference">
          „Ze začátku mi to přišlo dlouhé – 2 roky! Ale nesmírně rychle to
          uteklo a bylo to velmi přínosné. Moc se mi líbily příklady z dulí
          praxe.“
        </p>
        <p className="reference">
          „Děkuji za šanci výcvik absolvovat, pracovat na sobě a za čas, který
          jste nám všechny věnovaly.“
        </p>
        <h2>Jak náš výcvik probíhá</h2>
        <p>
          Výcvik je rozdělený na dva roky, které se od sebe odlišují jak náplní
          a programem, tak lektory. Každý z nich vám dá něco jiného nejen
          profesně, ale i, jak říkají samy studentky, také osobně.
        </p>
        <p>
          První rok je zaměřený především na teorii. Lektorky z Českých dul
          studentkám předávají odborné informace o tématech zabývajících se
          hranicemi profese, průběhem a fyziologií porodu, kojením,
          šestinedělím, nastavením spolupráce s klienty nebo dobrou komunikací s
          nimi či zdravotnickým personálem. Během tohoto roku se studentky
          setkají se zajímavými osobnostmi z oblasti porodní asistence,
          laktačního poradenství, práva nebo i zahraničními hosty zabývajícími
          se podporou žen v těchto obdobích. Mezi hostující lektory patří
          porodní astistentky, lékaři, duly, právníci nebo psychologové.
        </p>
        <p>
          Tento rok může částečně probíhat online. Jsme rády, že je takto výcvik
          přístupnější jak pro ženy, co dojíždí velkou vzdálenost, tak i pro
          ženy s malými miminky či před porodem.
        </p>
        <img src="img/pexels4.jpg" alt="" width="150px" />
        <p className="reference">
          „Jednoduše pozitivní. Nové vztahy, nové vize, nová přátelství, sdílení
          a intimita.“
        </p>
        <p className="reference">
          „Výcvik je koncipován profesionálně, ale zároveň pozitivně a vtipně.
          Lektorky byly na lekce perfektně připravené. Jsme moc šťastná, že jsem
          se pro výcvik rozhodla.“
        </p>
        <p>
          Druhý rok studia se věnuje takzvané sebezkušenosti. Tato část našeho
          výcviku je vedena zkušenými psychoterapeuty, pracujícími formou Pesso
          – Boyden Systém Psychomotor. Během pěti víkendů se budoucí duly v
          bezpečném prostředí osobnostně rozvíjí v několika duševních oblastech
          pomocí scénických, dramatických a tělově kontaktních postupů. Díky
          tomu se studentky naučí lépe zvědomovat vlastní prožívání. Naučí se
          rozpoznávat své vlastní emoce, pocity a tělové reakce, které se v
          interakci s dalšími lidmi objevují, a umět s nimi pracovat.
        </p>
        <p>
          Vše probíhá v jemnosti, pod dohledem a za laskavého vedení
          profesionálů. Navzdory častým počátečním obavám to nakonec mezi
          studentkami bývá velmi oblíbená část kurzu.
        </p>
        <img src="img/pexels1.jpg" alt="" width="150px" />
        <p className="reference">
          „Nevěděla jsem, co si představit pod slovem sebezkušenost. Ale nakonec
          mě praktická cvičení překvapila tím, jaké pocity po nich každá z nás
          měla a jak je každá jedna vnímala odlišně.“
        </p>
        <p className="reference">
          „Je mi líto, že výcvik už skončil. Celá sebezkušenostní část mi toho
          dala tolik, že mi pomáhá i v osobním životě.“
        </p>
        <h2>Co dalšího se děje během výcviku</h2>
        <h5>Intervize</h5>
        <p>
          Již během prvního roku jsou součástí výcviku povinné intervize. V
          menších, intervizních, skupinách, se studentky pravidelně setkávájí,
          aby si společně ukotvily témata z jednotlivých víkendů. Díky tomu si
          mohou vzájemně vyjasnit případné nejasnosti, prodiskutovat různá
          témata, popovídat si o doporučených knihách a jejich obsahu, a
          společně se tak začít v poli doprovázení orientovat. Intervize jsou
          nedílnou a velmi důležitou součástí výcviku mimo jiné také proto, že
          během nich studentky navazují kontakty a přátelství, která jsou v
          tomto oboru velmi důležitá.
        </p>
        <h5>Supervize</h5>
        <p>
          Od ukončení prvního roku výcviku je možné zúčastnit se také
          supervizních setkání. Ta jsou vedená nestranným odborníkem….Slouží k…
          Pro účast u závěrečné zkoušky je nutné absolvovat mininálně 3
          supervizní setkání.
        </p>
        <h5>Setkání s praktikujícími dulami</h5>
        <p>
          Během většiny víkendů mají studentky příležitost potkat se se zkušenou
          dulou. Ta jednak zasadí víkendové téma do konkrétních příkladů z praxe
          a zároveň studentkám odpovídá na jakékoliv jejich otázky ohledně
          života a práce duly.
        </p>
        <h5>Individuální terapie</h5>
        <p>
          V průběhu výcviku a před absvolvováním závěrečné zkoušky je nutné
          absolvovat alespoň 5 hodin individuálnách terapií. Je výhodou, pokud
          alespoň část z nich studentky absolvují souběžně se sebezkušenostní
          částí výcviku, která může přinášet a otevírat různá témata. Navazující
          terapie může pomoci vybudovat dobré základy pro budoucí profesi duly.
        </p>
        <h5>První doprovody s konzultantkou</h5>
        <p>
          Součástí výcviku a také nutnou podmínkou pro splnění závěrečné zkoušky
          jsou 3 samostatné doprovody u porodu. Každá studentka má svou
          konzultantku, profesně zkušenější dulu, která ji prvními doprovody
          provází. Studentky se na své konzultantky mohou s čímkoliv obracet,
          prochází s nimi celým obdobím spolupráce s klienty včetně samotného
          doprovodu u porodu. Konzultantky studentkám poskytují nejen informace,
          komplexní náhled na konkrétní situace a podporu, ale také zpětnou
          vazbu.
        </p>
        <h5>Závěrečná zkouška</h5>
        <p>
          Při splnění všech podmínek je výcvik zakončen závěrečnou zkouškou.
          Jejím splněním se studentky stávají certifikovanými dulami.{' '}
        </p>
        <div className="education-card">
          <p>Zde vám přinášíme shrnutí, co vás v našem výcviku čeká: </p>
          <ul>
            <li>11 víkendových setkání</li>
            <li>154 hodin výcviku</li>
            <li>70 hodin sebezkušenosti</li>
            <li>30 hodin intervize</li>
            <li>5 hodin individuální terapie</li>
            <li>3 skupinové supervize</li>
            <li>3 konzultované doprovody k porodu</li>
          </ul>
        </div>
        <img src="img/pexels2.jpg" alt="" width="150px" />
        <p className="reference">
          „Ještě stále mi úplně nedochází, jak moc mě výcvik změnil. Rozhodně mé
          nejlepší rozhodnutí, co se týče vzdělání. Úžasný seberozvoj, skvělé
          ženy, podpora, bezpečí. Děkuji z celého srdce.“
        </p>
        <p className="reference">
          „Organizace kurzu je velmi dobrá a vede ho skvělý tým. Během
          sebezkušenosti jsme měli úžasné lektory a poznatky z ní využívám i v
          běžném životě.“
        </p>
        <h2>Jak se přihlásit</h2>
        <p>
          Jestli máte jasno a chcete se do výcviku přihlásit, vyplňte motivační
          dopis ZDE a my vás moc rády pozveme na přijímací řízení.
        </p>
        <p>Příští výcvik je plánovaný na rok 2025 – 2027</p>
        <p>
          Dostalo se vám málo informací? Chcete toho o výcviku u Českých dul
          vědět víc? Nebojte se nás kontaktovat a na cokoli zeptat: Kontaktní
          formulář (Link do Kontaktů)
        </p>
        <img src="img/pexels1.jpg" alt="" width="150px" />
      </div>
    </>
  );
};
