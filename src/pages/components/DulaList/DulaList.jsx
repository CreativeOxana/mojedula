...
export default function DulaList({ region }) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const normalizedData = data.map((dula) => {
          ...
          // Pokud je language pole
          if (Array.isArray(languages)) {
            // Rozdělit každý prvek pole, pokud obsahuje čárku
            languages = languages.flatMap((lang) =>
              lang.includes(',') ? lang.split(',').map((l) => l.trim()) : lang,
            );
          } else {
            // Pokud je language string, rozdělit na pole
  ...
  // Nejprve filtrovat podle regionu
  const dulasByRegion = dulas.filter(
    (dula) => region === '' || dula.regionIds.includes(region),
  );

  // Získat unikátní jazyky pouze z dul ve vybraném regionu
  const allLanguages = [
    ...new Set(dulasByRegion.flatMap((dula) => dula.language)),
  ].sort();

  // Pak filtrovat podle jazyka
  const currentDulas = dulasByRegion.filter(
    (dula) =>
      selectedLanguage === '' || dula.language.includes(selectedLanguage),
  );
      ...
      <div className="dula-list-container">
        {currentDulas.length === 0 ? (
          <p className="no-results">
            Žádné duly nenalezeny pro vybrané filtry.
          </p>
        ) : (
